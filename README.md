...
方形主视觉与封面都使用与项目内容和使用流程相符的真实场景背景，背景满铺、文案叠加。场景可由图片工具生成，无需自行上传实拍底图；不会用纯黑、纯色、纯渐变或抽象图形替代，也不会给所有项目套用同一办公桌。极简、霓虹或 3D 预设仍遵循这项要求；独立项目图标不受此限制。
...
以下两张图例为 BrandLoom Skill 生成的图例。

<p align="center">
  <img src="docs/examples/brandloom-host-logo-card-1254x1254.png" alt="1254×1254 方形 LOGO 主视觉图例" width="80%">
</p>

<p align="center">
  <img src="docs/examples/brandloom-host-cover-1774x887.png" alt="1774×887 横向封面图例" width="80%">
</p>

- 方形 LOGO 主视觉：1254×1254（1:1），满铺背景，不含人物或 IP 形象。
- 横向封面：1774×887（约 2:1），满铺背景，包含 author-anime、tuotuo 和 xingbi，文案叠加在场景上。
...
- 所有待确认阶段都会在同一条用户消息中显示完整、互斥的选项，并明确标注一个结合项目内容给出的推荐选项。
- 用户只回复“继续”而没有选择字母时，Skill 会重新展示当前选项，不会猜测或跳过确认。
- 补齐 16 个待确认状态的可见菜单；风格档案、输出方案和公司 LOGO 权利/保存范围确认改为与实际 CLI 和 manifest 字段一致。
- 增加对应的 Skill 契约测试，覆盖选项完整性、推荐理由、互斥关系和权利字段边界。
