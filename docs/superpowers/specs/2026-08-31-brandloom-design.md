...
当前背景要求以文末「2026-09-09 真实场景契约修订」为准；下述风格是色调、光线与前景表现，不得解释为允许无真实场景的方形主视觉或封面。
...
宿主内置图片工具的稳定返回尺寸现作为新的默认契约：`logo-card` 为 1254 × 1254（1:1），`cover` 为 1774 × 887（2:1）。该修订覆盖新生成、模板渲染与离线 QA 默认值；GitHub Social Preview 仍为 1280 × 640。自定义尺寸仍可使用，但必须重新检查比例、安全区与可读性，且不得通过缩放无效宿主返回伪造验收。本文前述 2048 × 2048 与 2048 × 1024 数值保留为历史证据，不代表当前默认值。

### 2026-09-09 真实场景契约修订

用户确认：以后生成的方形 LOGO 主视觉与封面都必须根据项目内容选择适用的真实场景背景。本修订覆盖第 12、16、21、22、23 节中可能被解释为纯色/抽象背景、固定办公场景或左右分割布局的旧表述。

- `logo-card` 与 `cover` 使用可辨识的写实空间、相关物件/动作、可信光线/材质/透视；可以是 AI 生成的场景，不要求用户提供实拍照片，也不冒充真实地点或产品截图。
- 背景满铺，文案覆盖低干扰区域；不用纯黑、纯色、纯渐变、悬浮图标或抽象科技线条代替真实环境，不强制一边空白、一边产品场景。
- 场景依据已确认的用途、目标用户与使用流程选择，不给所有项目套用通用办公桌；信息不足时先确认有依据的候选场景，不虚构功能。
- 所有风格预设均服从此规则，极简/黑金/霓虹/3D 只影响色调、光线、排版和前景元素。独立 `project-mark` 和正式 `company-logo` 原素材不受这项背景规则影响。
- 在现有 `SHOT_LIST_PENDING` 确认场景及关联理由，分别保存到 JSON `style.scene` 与 `style.scene_rationale`；不新增状态或运行时依赖。方形与封面保持同一场景主题，本地化使用同一底图。
- 两类输出都将项目上下文和已确认场景传入底图提示词。内部 QA 与用户验收需检查实际视觉语义，不把尺寸、哈希或提示词文字通过当作真实场景验收通过。不合格时返回调整与确认，不自动重试、覆盖历史或交付。
