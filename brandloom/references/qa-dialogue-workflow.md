...
- `SHOT_LIST_PENDING`：按 `composition-recipes.md` 为方形与封面列出具体真实场景、项目关联及满铺构图，并将确认内容保存为 `style.scene` / `style.scene_rationale`；确认推荐；调整 LOGO 主视觉；调整封面；调整功能点或角色动作；只交付方案和提示词。缺少用途信息时先确认候选真实场景，不能以纯黑或抽象底图代替。
