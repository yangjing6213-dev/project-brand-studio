...
每个 `*_PENDING` 回合必须在同一条面向用户的消息中完整列出互斥选项（使用 A/B/C 等标签），随后明确标出一个推荐项，并根据当前项目内容说明推荐理由；最后请用户回复选项字母。不得把选项只写在工具输出、内部状态或默认推断中；不得只写确认结论而省略选项。用户只说“继续”但未选择选项时，必须重新显示选项并请求明确选择，不得替用户猜测。
...
`logo-card` 方形主视觉与 `cover` 封面都必须采用与项目内容、目标用户和使用流程相符的真实场景背景（可由图片工具生成的写实场景，不要求用户提供实拍照片）。背景满铺，文字覆盖在场景上；不能用纯黑、纯色、纯渐变或纯抽象图形替代，也不把所有项目套入同一办公桌。按 `references/composition-recipes.md` 先确认场景及其项目关联，再生成；风格预设不能覆盖此要求。独立 `project-mark` 图标与正式 `company-logo` 原文件不受场景背景规则影响。
